...
<div class="container">
    <div class="header">
        <div class="actions">
            ...
            {#if frame.paused}
                <button on:click={() => {setPaused(false)}}><i class="fas fa-pause-circle fa-fw"></i></button>
                {:else}
                <button class="inactive" on:click={() => {setPaused(true)}}><i class="fas fa-pause-circle fa-fw"></i></button>
            {/if}
    ...
    <div class="main">
        ...
        <button on:click={() => {setUrl(frame.url)}}><i class="fas fa-sign-in-alt fa-fw"></i></button>
        <button on:click={addPreset}><i class="fas fa-cloud-download-alt fa-fw"></i></button>
    </div>
        ...
        {#each presets as preset, i}
            {#if preset.name}
                <button
                    ...
                    on:click={presetClick} data-url={preset.url}><span>{preset.name}</span></button>
            {:else}
                <button
                    in:fly="{{  y: -50, duration: 100, delay: i * 100 }}"
                    out:fly="{{ y: 50, duration: 200, delay: i * 100 }}"
                    on:click={presetClick} data-url={preset.url}><span>{preset.url}</span></button>
            {/if}
        {/each}
    </div>
</div>
